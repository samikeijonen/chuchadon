...
function chuchadon_customize_register( $wp_customize ) {
	...
	if( class_exists( 'Jetpack' ) && current_theme_supports( 'jetpack-testimonial' ) ) {
		...
		$wp_customize->add_control(
			'hide_testimonials',
			array(
				'label'       => esc_html__( 'Hide testimonials', 'chuchadon' ),
				'description' => esc_html__( 'Check this if you want to hide testimonials from the Front Page Template.', 'chuchadon' ),
				'section'     => 'front-page',
				'priority'    => 70,
				'type'        => 'checkbox'
			)
		...
		$wp_customize->add_control(
			'order_testimonials',
			array(
				'label'       => esc_html__( 'Testimonials after featured area', 'chuchadon' ),
				'description' => esc_html__( 'Check this if you want to move testimonials after Featured area in the Front Page Template.', 'chuchadon' ),
				'section'     => 'front-page',
				'priority'    => 75,
				'type'        => 'checkbox'
			)
	...
	$front_page_featured_choices = array( 
		...
		'portfolios'  => esc_html__( 'Portfolios', 'chuchadon' )
	);
	...
	$wp_customize->add_control(
		...
		array(
			...
			'description' => esc_html__( 'Select do you want to feature Child Pages, Blog Posts or Portfolios in Front Page.', 'chuchadon' ),
			'section'     => 'front-page',
			'priority'    => 80,
			'type'        => 'radio',
	...
	$wp_customize->add_setting(
		...
		array(
			'default'           => apply_filters( 'chuchadon_default_bg_color', '#2a2a2a' ),
...
function chuchadon_color_backgrounds_css() {
	...
	/* Get header colors. */
	$header_bg_color = get_theme_mod( 'header_background_color', apply_filters( 'chuchadon_default_bg_color', '#2a2a2a' ) );
	...
	/* Header bg styles. */	
	if ( '#2a2a2a' !== $header_bg_color || 75 !== $header_bg_color_opacity ) {
...
function chuchadon_callout_output( $placement ) {
	...
	if ( get_theme_mod( 'callout_title_' . $placement ) || get_theme_mod( 'callout_url_' . $placement ) || get_theme_mod( 'callout_url_text_' . $placement ) || get_theme_mod( 'callout_text_' . $placement ) ) {
		...
		/* Start output. */
		$output .= '<div class="chuchadon-callout chuchadon-callout-' . $placement . '"><div class="entry-inner">';
